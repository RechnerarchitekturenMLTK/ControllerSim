...
public class Simulation {
			...
			if (start = true) {
				...
				while (scan.hasNextLine()) { // Prüft ob es eine nächste Zeile gibt, die noch nicht gelesen wurde
					...
					if (s1.substring(0, 1).isBlank()) {
						/*
						 * Prüft ob an erster und zweiter Stelle kein Wert steht, falls ja, wird der
						 * nächste While-Zyklus eingeleitet und die Nächste Zeile wird bearbeitet, falls
						 * nein wird die else-Klammer ausgeführt
						 */
					} else {
						...
						if (datenspeicher[4] != 0 && dec != 12 && dec != 13) {
							if (dec == 4) {
								...
								if (destBit == 0) {
									...
									datenspeicher[(int) t] = Math.floorMod(datenspeicher[(int) t], 256);
									System.out
											.println("rrf (f-register) " + datenspeicher[(int) t] + " cFlag: " + cFlag);
								}
							}
						} else {
	...
	public static void dateiSchreiben() {
		...
		try {
			...
			pWriter.println("Time " + (currentTime - startTime) + "ms");
